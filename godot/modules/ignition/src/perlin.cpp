...
double noise( double px, double py, double pz )
{
	...
	const double s = (px + py + pz) * F3; // Very nice and simple skew factor for 3D
	const int i = fastfloor(px + s);
	const int j = fastfloor(py + s);
	const int k = fastfloor(pz + s);

	const double t = (i + j + k) * G3;
	const double X0 = i - t; // Unskew the cell origin back to (x,y,z) space
	const double Y0 = j - t;
	const double Z0 = k - t;
	const double x0 = px - X0; // The x,y,z distances from the cell origin
	const double y0 = py - Y0;
	const double z0 = pz - Z0;

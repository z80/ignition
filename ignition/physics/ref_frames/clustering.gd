...
static func cluster_score( objs: Array, split_ind: int ) -> float:
	...
	for i in range(upper_bound):
		var body_a: RefFrameNode = objs[i]
		var lower_bound: int = i+1
		for j in range(lower_bound, split_ind):
			var body_b: RefFrameNode = objs[j]
...
static func cluster_swap( src: Array, element_ind: int, split_ind: int ) -> Array:
	...
	if element_ind < split_ind:
		...
		if element_ind < last_ind:
			var a: RefFrameNode = dest[element_ind]
			var b: RefFrameNode = dest[last_ind]
			dest[last_ind] = a
			dest[element_ind] = b
		split_ind -= 1
	else:
		...
		if first_ind < element_ind:
			var a: RefFrameNode = dest[first_ind]
			var b: RefFrameNode = dest[element_ind]
			dest[first_ind] = b
			dest[element_ind] = a
		split_ind += 1

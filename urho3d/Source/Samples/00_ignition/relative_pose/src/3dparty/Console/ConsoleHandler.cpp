...
void ConsoleHandler::SubscribeToEvents()
{
    ...
    SubscribeToEvent(IgnEvents::E_CONSOLE_COMMAND_ADD, URHO3D_HANDLER(ConsoleHandler, HandleConsoleCommandAdd));
    SubscribeToEvent(IgnEvents::E_CONSOLE_GLOBAL_VARIABLE_CHANGE, URHO3D_HANDLER(ConsoleHandler, HandleConsoleGlobalVariableChange));
    ...
    SendEvent(IgnEvents::E_CONSOLE_COMMAND_ADD, IgnEvents::ConsoleCommandAdd::P_NAME, "map", IgnEvents::ConsoleCommandAdd::P_EVENT, "#map", IgnEvents::ConsoleCommandAdd::P_DESCRIPTION, "Change the map");
    SubscribeToEvent("#map", [&](StringHash eventType, VariantMap& eventData) {
        ...
        SendEvent(IgnEvents::E_SET_LEVEL, data);
    });

    SendEvent(IgnEvents::E_CONSOLE_COMMAND_ADD, IgnEvents::ConsoleCommandAdd::P_NAME, "opacity", IgnEvents::ConsoleCommandAdd::P_EVENT, "#opacity", IgnEvents::ConsoleCommandAdd::P_DESCRIPTION, "Change the console opacity");
...
void ConsoleHandler::HandleConsoleGlobalVariableChange(StringHash eventType, VariantMap& eventData)
{
    ...
    if (params.Size() == 2) {
        ...
        using namespace IgnEvents::ConsoleGlobalVariableChanged;
        VariantMap data = GetEventDataMap();
        data[P_NAME] = params[0];
        data[P_VALUE] = newValue;
        SendEvent(IgnEvents::E_CONSOLE_GLOBAL_VARIABLE_CHANGED, data);
    }
}

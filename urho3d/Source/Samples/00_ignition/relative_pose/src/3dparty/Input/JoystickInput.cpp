...
void JoystickInput::HandleAxisMove(StringHash eventType, VariantMap& eventData)
{
	...
	if (buttonId == _joystickMapping.y_) {
		auto* controllerInput = GetSubsystem<ControllerInput>();
		if (position < 0) {
            controllerInput->SetActionState(Ign::CTRL_FORWARD, true, joystick);
            controllerInput->SetActionState(Ign::CTRL_BACK, false, joystick);
		}
		else if (position > 0) {
            controllerInput->SetActionState(Ign::CTRL_FORWARD, false, joystick);
            controllerInput->SetActionState(Ign::CTRL_BACK, true, joystick);
		}
		else {
            controllerInput->SetActionState(Ign::CTRL_FORWARD, false, joystick);
            controllerInput->SetActionState(Ign::CTRL_BACK, false, joystick);
	...
	if (buttonId == _joystickMapping.x_) {
		auto* controllerInput = GetSubsystem<ControllerInput>();
		if (position < 0) {
            controllerInput->SetActionState(Ign::CTRL_LEFT, true, joystick);
            controllerInput->SetActionState(Ign::CTRL_RIGHT, false, joystick);
		}
		else if (position > 0) {
            controllerInput->SetActionState(Ign::CTRL_LEFT, false, joystick);
            controllerInput->SetActionState(Ign::CTRL_RIGHT, true, joystick);
		}
		else {
            controllerInput->SetActionState(Ign::CTRL_LEFT, false, joystick);
            controllerInput->SetActionState(Ign::CTRL_RIGHT, false, joystick);

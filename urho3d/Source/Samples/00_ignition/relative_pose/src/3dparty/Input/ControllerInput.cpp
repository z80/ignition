	...
	_activeAction(-1)
{
    ...
    _controlMapNames[Ign::CTRL_FORWARD]  = "Move forward";
    _controlMapNames[Ign::CTRL_BACK]     = "Move backward";
    _controlMapNames[Ign::CTRL_LEFT]     = "Strafe left";
    ...
    _controlMapNames[Ign::CTRL_TRIGGER]   = "Trigger";
	Init();
}
...
{
    SubscribeToEvent(IgnEvents::E_START_INPUT_MAPPING, URHO3D_HANDLER(ControllerInput, HandleStartInputListening));
...
void ControllerInput::SetConfiguredKey(int action, int key, String controller)
{
    ...
    using namespace IgnEvents::InputMappingFinished;
    ...
    SendEvent(IgnEvents::E_INPUT_MAPPING_FINISHED, data);
...
{
    using namespace IgnEvents::StopInputMapping;
    VariantMap data = GetEventDataMap();
    ...
    SendEvent(IgnEvents::E_STOP_INPUT_MAPPING, data);

}

void ControllerInput::HandleStartInputListening(StringHash eventType, VariantMap& eventData)
{
    using namespace IgnEvents::StartInputMapping;
...
void ControllerInput::HandleStartInputListeningConsole(StringHash eventType, VariantMap& eventData)
{
	...
	if (parameters.Size() == 2) {
        ...
        SendEvent(IgnEvents::E_START_INPUT_MAPPING, data);
		return;
	}
...
void ControllerInput::CreateController(int controllerIndex)
{
    ...
    using namespace IgnEvents::ControllerAdded;
	_controls[controllerIndex] = Controls();
	VariantMap data = GetEventDataMap();
	data[P_INDEX] = controllerIndex;
    SendEvent(IgnEvents::E_CONTROLLER_ADDED, data);
...
void ControllerInput::DestroyController(int controllerIndex)
{
	...
	if (controllerIndex > 0) {
        ...
        using namespace IgnEvents::ControllerRemoved;

		VariantMap data = GetEventDataMap();
		data[P_INDEX] = controllerIndex;
        SendEvent(IgnEvents::E_CONTROLLER_REMOVED, data);
	}
	GetSubsystem<DebugHud>()->SetAppStats("Controls", _controls.Size());
}

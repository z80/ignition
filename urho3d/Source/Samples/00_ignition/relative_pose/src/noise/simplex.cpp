...
static inline int fastfloor(const Float x) { return int(x > 0 ? x : x - 1); }

//static Float dot( const int* g, const Float x, const Float y ) { return g[0]*x + g[1]*y; }
static inline Float dot(const Float *g, const Float x, const Float y, const Float z) { return g[0] * x + g[1] * y + g[2] * z; }
...
Float noise( const Vector3d & p )
{
    ...
    const int i = fastfloor(p.x_ + s);
    const int j = fastfloor(p.y_ + s);
    const int k = fastfloor(p.z_ + s);

	const Float t = (i + j + k) * G3;
	const Float X0 = i - t; // Unskew the cell origin back to (x,y,z) space
	const Float Y0 = j - t;
	const Float Z0 = k - t;
    const Float x0 = p.x_ - X0; // The x,y,z distances from the cell origin
    const Float y0 = p.y_ - Y0;
    const Float z0 = p.z_ - Z0;
	...
	const Float x2 = x0 - i2 + G3mul2; // Offsets for third corner in (x,y,z) coords
	const Float y2 = y0 - j2 + G3mul2;
	const Float z2 = z0 - k2 + G3mul2;
	const Float x3 = x0 - 1.0 + G3mul3; // Offsets for last corner in (x,y,z) coords
	const Float y3 = y0 - 1.0 + G3mul3;
	const Float z3 = z0 - 1.0 + G3mul3;

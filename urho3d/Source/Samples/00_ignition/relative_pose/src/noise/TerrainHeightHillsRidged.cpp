...
template <>
TerrainHeightFractal<TerrainHeightHillsRidged>::TerrainHeightFractal(const PiSourceDesc&body) :
	PiBodySource(body)
{
	//textures:
...
template <>
Float TerrainHeightFractal<TerrainHeightHillsRidged>::GetHeight(const Vector3d &p) const
{
	Float continents = ridged_octavenoise(GetFracDef(3), 0.65, p) * (1.0 - m_sealevel) - (m_sealevel * 0.1);
	if (continents < 0) return 0;
	Float n = continents;
	Float distrib = river_octavenoise(GetFracDef(4), 0.5, p);
	Float m = 0.5 * ridged_octavenoise(GetFracDef(4), 0.55 * distrib, p);

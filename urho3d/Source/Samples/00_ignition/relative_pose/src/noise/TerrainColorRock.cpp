...
template <>
TerrainColorFractal<TerrainColorRock>::TerrainColorFractal(const PiSourceDesc&body) :
	PiBodySource(body)
{
}

template <>
Vector3d TerrainColorFractal<TerrainColorRock>::GetColor(const Vector3d &p, Float height, const Vector3d &norm) const
{
	Float n = m_invMaxHeight * height / 2;
	if (n <= 0) return m_darkrockColor[0];
	const Float flatness = pow(p.DotProduct(norm), 20.0);
	const Vector3d color_cliffs = m_rockColor[0];
	const Float equatorial_desert = (2.0 - m_icyness) *
		(-1.0 + 2.0 * octavenoise(4, 0.05, 2.0, (n * 2.0) * p)) *
		1.0 * (2.0 - m_icyness) * (1.0 - p.y_ * p.y_);
	//Float equatorial_region = octavenoise(GetFracDef(0), 0.54, p) * p.y_ * p.x_;
	//Float equatorial_region_2 = ridged_octavenoise(GetFracDef(1), 0.58, p) * p.x_ * p.x_;

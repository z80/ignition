...
Vector3d TerrainColorFractal<TerrainColorGGNeptune2>::GetColor(const Vector3d &p, Float height, const Vector3d &norm) const
{
	...
	if (p.y_ < 0.5 && p.y_ > -0.5) {
		for (float i = -1; i < 1; i += 0.6f) {
			...
			if (temp < .07 + h && temp > -.07 + h) {
				n = 2.0 * billow_octavenoise(GetFracDef(2), 0.5 * m_entropy[0], piSimplex(Vector3d(p.x_, p.y_ * m_planetEarthRadii * 0.3, p.z_)) * p);
				n += 0.8 * octavenoise(GetFracDef(1), 0.5 * m_entropy[0], piSimplex(Vector3d(p.x_, p.y_ * m_planetEarthRadii, p.z_)) * p);
	...
	//if is not a stripe.
	n = octavenoise(GetFracDef(1), 0.5 * m_entropy[0] + 0.25f, piSimplex(Vector3d(p.x_ * 0.2, p.y_ * m_planetEarthRadii * 10, p.z_)) * p);
	//n += 0.5;

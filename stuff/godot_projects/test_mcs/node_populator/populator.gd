...
func update_sync( parent: Spatial, cubes: MarchingCubesDualGd, se3: Se3Ref, scaler: DistanceScalerBaseRef ):
	...
	for node_ind in node_indices:
		...
		if not has:
			for creator in creators:
				var created_instances: Array = _populate_node_sync( parent, cubes, node, creator, scaler )
...
func update( parent: Spatial, cubes: MarchingCubesDualGd, se3: Se3Ref, scaler: DistanceScalerBaseRef ):
	...
	for node_ind in node_indices:
		...
		if has:
			...
			for creator in creators:
				total_qty += 1
				var args: Array = [parent, cubes, node, creator, scaler, populated_node_paths]
...
func _populate_node_sync( parent: Spatial, cubes: MarchingCubesDualGd, node: MarchingCubesDualNodeGd, creator: Resource, scaler: DistanceScalerBaseRef ):
	...
	for i in range(qty):
		...
		var instance: Spatial = creator.create( node, se3, _rand, scaler )
		created_instances.push_back( instance )
		
		var t: Transform = instance.transform

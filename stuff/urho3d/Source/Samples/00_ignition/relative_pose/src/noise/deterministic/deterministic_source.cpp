...
DeterministicSource::DeterministicSource( const PiSourceDesc & body )
    : HeightSource(), 
    m_seed(body.seed_),
    m_rand(body.seed_),
    m_minh(0)
...
Float DeterministicSource::height( const Vector3d & at ) const
{
    return 0.0;
}

Color DeterministicSource::color( const Vector3d & at, const Vector3d & norm, Float height )  const
{
    return Color();
}

void DeterministicSource::setFracDef( const unsigned int index, const Float featureHeightMeters, const Float featureWidthMeters, const Float smallestOctaveMeters )
{

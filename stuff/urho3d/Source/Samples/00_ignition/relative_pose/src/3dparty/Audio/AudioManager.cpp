...
{
    SubscribeToEvent(IgnEvents::E_PLAY_SOUND, URHO3D_HANDLER(AudioManager, HandlePlaySound));
    SubscribeToEvent(IgnEvents::E_STOP_SOUND, URHO3D_HANDLER(AudioManager, HandleStopSound));
    SubscribeToEvent(IgnEvents::E_STOP_ALL_SOUNDS, URHO3D_HANDLER(AudioManager, HandleStopAllSounds));
...
void AudioManager::SubscribeConsoleCommands()
{
    ...
    SendEvent(IgnEvents::E_CONSOLE_COMMAND_ADD, data);
}

void AudioManager::HandlePlaySound(StringHash eventType, VariantMap& eventData)
{
    using namespace IgnEvents::PlaySound;
    int index = -1;
...
{
    using namespace IgnEvents::StopSound;
...
void AudioManager::HandleButtonClick(StringHash eventType, VariantMap& eventData)
{
    ...
    SendEvent(IgnEvents::E_PLAY_SOUND, data);
}
